...
const state = {
  memberships: loadMemberships(), // { [group_id]: {member_id, secret, name, group_name, invite_code} }
...
async function api(path, { method = "GET", body, auth = false } = {}) {
  ...
  if (auth) {
    const m = state.memberships[state.activeGroupId];
    if (!m) throw new Error("Not joined to this tab yet");
    headers["X-Member-Id"] = m.member_id;
    headers["X-Member-Secret"] = m.secret;
  }
...
function init() {
  ...
  if (code) {
    // Already a member of this group - just switch to it.
